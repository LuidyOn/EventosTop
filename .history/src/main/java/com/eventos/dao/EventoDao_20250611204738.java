...
public class EventoDao {
    ...
    public Evento criarEvento(Evento evento) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            // (stmt.setString(3, evento.getData().toString()); // Linha anterior
            stmt.setDate(3, java.sql.Date.valueOf(evento.getData())); // Linha corrigida
    ...
    public Optional<Evento> atualizarEvento(Evento evento) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            stmt.setString(1, evento.getNome());
            stmt.setString(2, evento.getDescricao());
            stmt.setDate(3, java.sql.Date.valueOf(evento.getData())); // <<< CORRIGIDO AQUI
            stmt.setString(4, evento.getLocal());
            stmt.setInt(5, evento.getCapacidade());
    ...
    public void associarPalestrante(int eventoId, int palestranteId) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            // Lançar RuntimeException se a associação já existir (chave primária duplicada)
            if (e.getSQLState().startsWith("23")) { // SQLState para violação de integridade (duplicate key)
                throw new RuntimeException("Erro: Este palestrante já está associado a este evento.");
            }
    ...
    // Adicione este método para remover um único palestrante de um evento
